...
export class XR83Image extends THREE.Mesh {
  ...
  onImageLost(detail) {
    detail.metadata.entity.visible = false;
  }
}

export class XR8Video extends THREE.Mesh {
  constructor(videoSrc, width = 1, height = 1) {
    const domVideo = document.createElement('video');
    const videoTexture = new THREE.VideoTexture(domVideo);
    ...
    this.video = domVideo
    this.video.src = videoSrc; // TODO discover the ratio of the video instead of relying on supplied values in the constructor
    ...
    this.visible = false;
    console.log('[XR8Video] constructed, src:', videoSrc, 'visible:', this.visible, 'video.readyState:', this.video.readyState);
  }

  onImageFound(detail) {
    console.log('[XR8Video] onImageFound called!', 'name:', detail.name, 'position:', detail.position, 'scale:', detail.scale);
    applyTargetPose(this, detail);
    this.visible = true;
    this.video.play().then(() => {
      console.log('[XR8Video] video playing, readyState:', this.video.readyState, 'videoWidth:', this.video.videoWidth, 'videoHeight:', this.video.videoHeight);
      console.log('[XR8Video] mesh visible:', this.visible, 'position:', this.position.toArray(), 'scale:', this.scale.toArray());
      console.log('[XR8Video] material.map:', this.material.map, 'material.visible:', this.material.visible);
    }).catch((err) => console.warn('[XR8Video] Video play blocked:', err));
  }
  onImageUpdated(detail) {
    applyTargetPose(this, detail);
  }
  onImageLost() {
    console.log('[XR8Video] onImageLost called');
    this.visible = false;
    this.video.pause();
  }
}
...
// TODO
export class XR8ScreenedVideo extends XR8Video {
  constructor(videoSrc, filteredColor = "#00FF00", width = 1, height = 1) {
    super(videoSrc);
  }
}

export class XR8Model extends THREE.Object3D {
  constructor(model) {
    super();
    this.add(model);
    this.visible = false;
